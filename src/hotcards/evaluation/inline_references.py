...
def _reference_snapshot(asset_id: str) -> ImageReferenceSnapshot:
    return ImageReferenceSnapshot(
        card_id=uuid5(NAMESPACE_URL, f"hotcards-eval:{asset_id}:card"),
        revision_id=uuid5(NAMESPACE_URL, f"hotcards-eval:{asset_id}:revision"),
        background_id=uuid5(NAMESPACE_URL, f"hotcards-eval:{asset_id}:background"),
    )
...
<title>HotCards blind inline Reference review</title>
<style>
