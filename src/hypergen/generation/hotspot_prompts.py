...
class ModelPolygon(DomainModel):
    ...
    points: tuple[ModelPoint, ...] = Field(min_length=3, max_length=16)
...
class ModelInteractionOutput(DomainModel):
    ...
    polygons: tuple[ModelPolygon, ...] = Field(min_length=1, max_length=4)
...
- Return each described interaction exactly once, with at most 12 interactions, 4 polygon
  components per interaction, and 16 points per component.
- Keep polygons simple and reasonably editable; do not create holes.
- For destinations, select only a supplied opaque card token, propose a human-readable new card
  name, or mark the destination unresolved.
...
class OllamaHotspotGenerator:
    ...
    def generate(self, request: HotspotGenerationRequest) -> HotspotGenerationResult:
        ...
        for proposal in proposals:
            signature = proposal.model_dump_json(exclude={"warnings"})
            if signature in seen_proposals:
                warnings.append(f"model repeated hotspot proposal {proposal.label!r}")
            seen_proposals.add(signature)
        if len(output.interactions) == 12:
            warnings.append(
                "model response reached the preliminary 12-interaction limit; "
